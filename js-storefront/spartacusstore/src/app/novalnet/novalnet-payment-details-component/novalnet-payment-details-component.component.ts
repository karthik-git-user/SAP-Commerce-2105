...
import { Country, GlobalMessage, GlobalMessageService, GlobalMessageType, HttpErrorHandler, OccEndpointsService, UserAddressService, UserPaymentService } from '@spartacus/core';
import { BehaviorSubject, combineLatest, Observable, Observer, throwError } from 'rxjs';
...
// const SCRIPT_PATH = 'https://adminbeta.novalnet.de/js/V13PaymentFormCheckout2.js?'+Math.floor(Math.random()*999);
...
@Component({
  selector: 'app-novalnet-payment-details-component',
  templateUrl: './novalnet-payment-details-component.component.html',
  styleUrls: ['./novalnet-payment-details-component.component.scss'],
...
export class NovalnetPaymentDetailsComponentComponent {
  ...
  constructor(userPaymentService: UserPaymentService, checkoutDeliveryAddressFacade: CheckoutDeliveryAddressFacade, fb: FormBuilder, userAddressService: UserAddressService, occEndpoint:OccEndpointsService,  http:HttpClient,  sanitizer: DomSanitizer, scriptService: ScriptService, renderer: Renderer2, activeCartService: ActiveCartService, novalnetService: NovalnetService, checkoutStepService: CheckoutStepService, activatedRoute :ActivatedRoute, errorHandler: HttpErrorHandler, globalMessageService:GlobalMessageService, launchDialogService: LaunchDialogService, vcr : ViewContainerRef) {
    ...
    // this.userPreferences$ = Observable<any>;
  ...
  ngOnInit () {
	  ...
	  //~ this.listener = this.renderer.listen("window", "onProcessCompletion", event => {
    ...
    // this.userPreferences$ = new Observer(function subscribe(subscriber) {subscriber.next(true);});
    // this.showPayments$ = this.getPaymentInfo().subscribe((response: any) => this.paymentResponse = response);
  ...
  setchecked(event: any) {
    ...
    // this.novalnetService.setPaymentType(this.currentChecked);
  }
  ...
  loadiframe() {
    ...
    //~ this.novalnetpaymentForm.walletResponse( (response:any) => {console.log(response) });
    //~ this.novalnetpaymentForm.onProcessCompletion( (response:any) => {console.log(response) });
    
  }
  

	
	@HostListener('window:onProcessCompletion', ['$event'])
  onMouseUp1(event:any) {
    console.log(event);
    console.log("test777777777");
  }
  
  	//~ @HostListener('novalnetpaymentForm:onProcessCompletion', ['$event'])
  //~ onMouseUp2(event) {
    //~ console.log(event);
    //~ console.log("test5555555555");
  //~ }
  ...
  async showPaymentForm() {
    ...
    if(!this.loadPaymentform) {
      ...
      if (this.billingAddressForm.valid || this.sameAsDeliveryAddress) {
        ...
        // var res=  this.http.post(requestURL, request).toPromise();
        // console.log(res);

      //   let t = await new Promise<any>(resolve =>  this.http.post(requestURL, request)
      // .subscribe( (res:any) => {
        
        //  this.htmliframe = '<div><br/><h3 class="cx-checkout-title d-none d-lg-block d-xl-block">  {{ "paymentForm.payment" | cxTranslate }}</h3><br/><iframe id = "novalnetPaymentIFrame" src="'+this.urlSafe+'" width="100%" id="novalnetPaymentIFrame" scrolling="no" (load) = "loadiframe()"></iframe></div>';
      //    this.urlSafe= this.sanitizer.bypassSecurityTrustResourceUrl(res.paymentURL);
      //    this.loadPaymentform = !this.loadPaymentform;
      //    console.log(res.paymentURL);
      //    resolve("loaded")
      // }));
      
      //   this.http.post(requestURL, request).subscribe((response:any) => {

      //   this.urlSafe= this.sanitizer.bypassSecurityTrustResourceUrl(response.paymentURL);
      ...
      // })
        ...
        // console.log(this.urlSafe);
        // this.loadPaymentform = !this.loadPaymentform;
        // console.log(this.loadPaymentform);
        // console.log("test");
      } else {
          this.billingAddressForm.markAllAsTouched();
      }
    } else {
      // this.novalnetpaymentForm.validationResponse((validation_response:any) => {console.log(validation_response+"===========88888888888");});
  ...
  async geturl() {
    ...
    let request = {};
      ...
      this.url = await new Promise<any>(resolve =>  this.http.post(requestURL, request)
    .subscribe( (res:any) => { resolve(res.paymentURL)}));
