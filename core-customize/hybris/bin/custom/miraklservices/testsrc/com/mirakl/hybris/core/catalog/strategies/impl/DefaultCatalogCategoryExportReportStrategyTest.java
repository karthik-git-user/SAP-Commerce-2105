...
public class DefaultCatalogCategoryExportReportStrategyTest {
  ...
  @InjectMocks
  private DefaultCatalogCategoryExportReportStrategy exportReportStrategy;
  ...
  @Mock
  private MiraklHierarchyImportResult exportResult1, exportResult2;
  @Captor
  ...
  public void setUp() {
    ...
    when(mciApi.getHierarchyImportResult(new MiraklHierarchyImportStatusRequest((JOB_ID_1)))).thenReturn(exportResult1);
    when(mciApi.getHierarchyImportResult(new MiraklHierarchyImportStatusRequest((JOB_ID_2)))).thenReturn(exportResult2);
  ...
  public void shouldUpdateCompletedExports() {
    when(miraklJobReportDao.findPendingJobReportsForType(CATALOG_CATEGORY_EXPORT))
  ...
  public void shouldHandleMiraklExceptions() {
    when(miraklJobReportDao.findPendingJobReportsForType(CATALOG_CATEGORY_EXPORT))
        .thenReturn(asList(pendingReport1, pendingReport2));
    when(mciApi.getHierarchyImportResult(new MiraklHierarchyImportStatusRequest(JOB_ID_1)))
